...
% planet1 = 'Earth';
% planet2 = 'Mars';
% tm = 1; Dovrebbe indicare con 1 prograde e con -1 retrograde
...
function [DV_1,DV_2] = interplanetary_transfer(date1,date2,planet1,planet2,tm)
    ...
    xfer_time = date2 - date1; %Il nostro time of flight
    time_array = linspace(date1,date2,10000); %Discretizzazione del ToF
    
    % calculate the planet state vectors at each timestep
    [p1_pos,p1_vel] = OrbitPropogator(time_array,planet1); %Qui potrei pensare di usare la funzione mia delle Efemeridi
    [p2_pos,p2_vel] = OrbitPropogator(time_array,planet2);

% [p1_pos,p1_vel] = getEphemeris('Earth',planet1);
% [p2_pos,p2_vel] = getEphemeris('317002',planet2);
